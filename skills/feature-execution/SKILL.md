...
5. **Codebase readiness check:** collect unique file paths from "Files to modify" across current session's tasks. Check that base directories exist (e.g., `src/`, `tests/`). If key directories are missing → **STOP**: "Каталог {path} не найден. Убедись, что проект инициализирован и путь к коду верный." Do NOT proceed into session scope confirmation with a missing codebase.
