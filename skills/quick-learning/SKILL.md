...
### Similarity Check (search-first)

Extract 3-4 key words from the new pattern's trigger and goal.
Search `$AGENTS_HOME/skills/quick-learning/references/triad-index.md` for those words.
...
Count unadapted triads: search for `| — |$` in triad-index.md (use `$` anchor — middle columns also contain `| — |`).
